...
export class MainContract implements Contract {
    ...
    static createFromConfig(config: MainContractConfig, code: Cell, workchain = 0) {
        ...
        // return new MainContract(address, config ? { ...init, ...config } : init);
    }
    ...
    async sendDesposit(provider: ContractProvider, sender: Sender, value: bigint) {
    ...
    async getData(provider: ContractProvider) {
        const { stack } = await provider.get("get_contact_storage_data", []);
        return {
